...
import com.coolGi.lod.wrappers.Matrix4fWrapper;
...
public class LodRenderer
{
	...
	public void drawLODs(LodDimension lodDim, PoseStack mcMatrixStack, float partialTicks, ProfilerFiller newProfiler)
	{
		...
		if (LodConfig.Client.Debug.debugMode == DebugMode.SHOW_DETAIL_WIREFRAME)
		...
		Matrix4f mcProjectionMatrix = new Matrix4fWrapper(mcProjMatrixRaw).floatArrayToMatrix();
		...
		if (mc.getClientWorld().dimensionType().hasCeiling())
			farPlaneBlockDistance = Math.min(LodConfig.Client.Graphics.QualityOption.lodChunkRenderDistance, LodUtil.CEILED_DIMENSION_MAX_RENDER_DISTANCE) * LodUtil.CHUNK_WIDTH;
		else
			farPlaneBlockDistance = LodConfig.Client.Graphics.QualityOption.lodChunkRenderDistance * LodUtil.CHUNK_WIDTH;
		...
		if (vbos != null)
		{
			...
			boolean cullingDisabled = LodConfig.Client.Graphics.AdvancedGraphicsOption.disableDirectionalCulling;
			boolean renderBufferStorage = LodConfig.Client.Graphics.AdvancedGraphicsOption.gpuUploadMethod == GpuUploadMethod.BUFFER_STORAGE && GlProxy.getInstance().bufferStorageSupported;
	...
	private void setupFog(FogDistance fogDistance, FogQuality fogQuality)
	{
		...
		if (fogDistance == FogDistance.FAR)
		{
			...
			if (fogQuality == FogQuality.FANCY)
			{
				...
				if (LodConfig.Client.Graphics.FogQualityOption.fogDistance == FogDistance.NEAR_AND_FAR)
			...
			int defaultFogMode, int defaultFogDistance)
	{
		...
		if (!fogSettings.vanillaIsRenderingFog
				|| LodConfig.Client.Graphics.FogQualityOption.disableVanillaFog)
		{
	...
	{
		Matrix4f lodPoj;
		float nearClipPlane = LodConfig.Client.Graphics.AdvancedGraphicsOption.useExtendedNearClipPlane ? vanillaBlockRenderedDistance / 5 : 1;
	...
	private NearFarFogSettings determineFogSettings()
	{
		...
		FogDrawOverride override = LodConfig.Client.Graphics.FogQualityOption.fogDrawOverride;
		...
		switch (quality) {
			case FANCY -> {
				...
				switch (LodConfig.Client.Graphics.FogQualityOption.fogDistance) {
					case NEAR_AND_FAR -> {
	...
	private void determineIfLodsShouldRegenerate(LodDimension lodDim, float partialTicks)
	{
		...
		if (ClientProxy.previousLodRenderDistance != LodConfig.Client.Graphics.QualityOption.lodChunkRenderDistance
					|| chunkRenderDistance != prevRenderDistance
					|| prevFogDistance != LodConfig.Client.Graphics.FogQualityOption.fogDistance)
		{

			vanillaRenderedChunks = new boolean[vanillaRenderedChunksWidth][vanillaRenderedChunksWidth];
			DetailDistanceUtil.updateSettings();
			fullRegen = true;
			previousPos = LevelPosUtil.createLevelPos((byte) 4, mc.getPlayer().chunkPosition().x, mc.getPlayer().chunkPosition().z);
			prevFogDistance = LodConfig.Client.Graphics.FogQualityOption.fogDistance;
			prevRenderDistance = chunkRenderDistance;
		}

		// did the user change the debug setting?
		if (LodConfig.Client.Debug.debugMode != previousDebugMode)
		{
			previousDebugMode = LodConfig.Client.Debug.debugMode;
			fullRegen = true;
		}
		...
		if (newTime - prevPlayerPosTime > LodConfig.Client.Buffers.rebuildTimes.playerMoveTimeout)
		{
			if (LevelPosUtil.getDetailLevel(previousPos) == 0
						|| mc.getPlayer().chunkPosition().x != LevelPosUtil.getPosX(previousPos)
						|| mc.getPlayer().chunkPosition().z != LevelPosUtil.getPosZ(previousPos))
			{
				vanillaRenderedChunks = new boolean[vanillaRenderedChunksWidth][vanillaRenderedChunksWidth];
				fullRegen = true;
				previousPos = LevelPosUtil.createLevelPos((byte) 4, mc.getPlayer().chunkPosition().x, mc.getPlayer().chunkPosition().z);
		...
		float minLightingDifference = switch (LodConfig.Client.Buffers.rebuildTimes) {
			case FREQUENT -> 0.025f;
			case NORMAL -> 0.05f;
			case RARE -> 0.1f;
		};
		...
		if (newTime - prevVanillaChunkTime > LodConfig.Client.Buffers.rebuildTimes.renderedChunkTimeout)
		{
		...
		if (newTime - prevChunkTime > LodConfig.Client.Buffers.rebuildTimes.chunkChangeTimeout)
		{
		...
		for (ChunkPos pos : chunkPosToSkip)
		{
			...
			xIndex = (pos.x - mc.getPlayer().chunkPosition().x) + (chunkRenderDistance + 1);
			zIndex = (pos.z - mc.getPlayer().chunkPosition().z) + (chunkRenderDistance + 1);
