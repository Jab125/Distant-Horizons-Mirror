...
import com.seibel.lod.wrappers.Matrix4fWrapper;
...
public class LodRenderer
{
	...
	public void drawLODs(LodDimension lodDim, PoseStack mcModelViewMatrix, float partialTicks, ProfilerFiller newProfiler)
	{
		...
		if (LodConfig.Client.Debug.drawLods)
		{
		...
		if (vbos != null && LodConfig.Client.Debug.drawLods)
		{
			...
			boolean renderBufferStorage = LodConfig.Client.Graphics.AdvancedGraphicsOption.gpuUploadMethod == GpuUploadMethod.BUFFER_STORAGE && glProxy.bufferStorageSupported;
...
//		GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, 0);
	...
	private void determineIfLodsShouldRegenerate(LodDimension lodDim, float partialTicks)
	{
		...
		if (newTime - prevPlayerPosTime > LodConfig.Client.AdvancedModOptions.Buffers.rebuildTimes.playerMoveTimeout)
		{
		...
		switch (LodConfig.Client.AdvancedModOptions.Buffers.rebuildTimes)
		{
			case FREQUENT:
		...
		if (newTime - prevVanillaChunkTime > LodConfig.Client.AdvancedModOptions.Buffers.rebuildTimes.renderedChunkTimeout)
		{
		...
		if (newTime - prevChunkTime > LodConfig.Client.AdvancedModOptions.Buffers.rebuildTimes.chunkChangeTimeout)
		{
